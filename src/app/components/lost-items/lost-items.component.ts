...
export class LostItemsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.apiService.getLostItemsByUsername(this.username).subscribe({
      next: (data: Item[]) => {
        ...
        if (this.items.length === 0) {
          this.snackBar.open('No lost items found for this user.', 'Close', {
            duration: 3000,
          });
        }
      },
      error: (error) => {
        this.snackBar.open('Failed to load lost items.', 'Close', {
          duration: 3000,
        });
      },
    });
  ...
  loadItems(): void {
    ...
    //     if (this.items.length === 0) {
    //       this.snackBar.open('No lost items found.', 'Close', {
    //         duration: 3000,
    //       });
    //     }
    //   },
  ...
  applyFilters(): void {
    ...
    this.searchItems = this.items.filter(
      (item) =>
        ...
        (this.filterDate
          ? new Date(item.dateLost).toISOString().startsWith(this.filterDate)
          : true)
    );
