  ...
  ngOnInit(): void {
    if(this.route.snapshot.params['id']){
      ...
      this.apiService.getFoundItemById(this.itemId).subscribe(data=>{
        this.item = data;
        // this.item.imageUrl = this.item.imageUrl ? `${baseUrl}${this.item.imageUrl}` : null
        console.log(this.item);
        this.foundItemForm.patchValue({
          item_name: this.item.item_name,
          category: this.item.category,
          description: this.item.description,
          found_location: this.item.found_location,
          date_found: this.item.date_found,
          contact_info: this.item.contact_info,
