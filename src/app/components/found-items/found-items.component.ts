    ...
    this.apiService.getFoundItems().subscribe({
      next: (data: FoundItem[]) => {
        ...
        if (this.items.length === 0) {
          this.snackBar.open('No found items reported.', 'Close', {
            duration: 3000,
          });
        }
      },
      error: (error) => {
        ...
        this.snackBar.open(
          'Failed to load found items. Check the console.',
          'Close',
          { duration: 3000 }
        );
      },
  ...
  applyFilters(): void {
    ...
    this.searchItems = this.items.filter(
      (item) =>
        ...
        (this.filterDate
          ? new Date(item.date_found).toISOString().startsWith(this.filterDate)
          : true)
    );
